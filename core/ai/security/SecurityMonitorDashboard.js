...
// Initialize configuration once at module level
...
class SecurityMonitorDashboard extends EventEmitter {
  ...
  _getPerformanceMetrics() {
    const metrics = {
      ...
      errorRate: this.performanceMetrics.errorCount / Math.max(1, this.performanceMetrics.requestCount),
  ...
  _checkPerformanceThresholds(metrics) {
    const { performanceThresholds } = this.config.errorMonitoring;

    if (metrics.errorRate > performanceThresholds.errorRate) {
      this._createPerformanceAlert('High Error Rate', {
    ...
    if (metrics.averageResponseTime > performanceThresholds.responseTime) {
      this._createPerformanceAlert('High Response Time', {
    ...
    if (memoryUsagePercent > performanceThresholds.memoryUsage) {
      this._createPerformanceAlert('High Memory Usage', {
   ...
   * @private
   */
  _createPerformanceAlert(type, details) {
    const alert = {
...
// Export the SecurityMonitorDashboard class
module.exports = SecurityMonitorDashboard;
